...
# save(co2_quadratic_tmp, file='1997_quad_seasonal_modelfit.RData')
...
arima_forecast <- fabletools::forecast(arima013011, h = "100 years", level = c(80, 95))
...
co2_420 <- ts_co2_new %>%
  forecast::forecast(arima013011, h = 10, level = c(80, 95)) %>%
  filter(.level == 80) %>%
  filter(value >= 420) %>%
  slice(1)
