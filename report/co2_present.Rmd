...
#NSA
# Using the ARIMA(1,0,4)(1,1,0) (chosen by AIC)
nsa.fc <- fabletools::forecast(autofit.arima.aic,h="2 years")
attr(autofit.arima.aic$auto[[1]]$fit$tsp$range, 'week_start') <- 7
...
sa.fc <- fabletools::forecast(autofit.arima.aic.sa,h="2 years")
attr(autofit.arima.aic.sa$auto[[1]]$fit$tsp$range, 'week_start') <- 7
sd <- tsibble(week=yearweek(seq(ymd('2019-12-29'), ymd('2023-01-01'), by ='1 week'), week_start = 7))
sd <- tsibble::fill_gaps(sd)
attr(sd$week, "week_start") <- 7
sd <- as_tsibble(sd)
# nsa.fc <- fabletools::forecast(autofit.arima.bic, new_data=nd)
sa.fc <- fabletools::forecast(autofit.arima.aic.sa, h="2 years")
sa.accuracy.forecast <- sa.fc %>% fabletools::accuracy(select(test.df, week, ppm))

rbind(nsa.accuracy.forecast, sa.accuracy.forecast) %>% 
  select(.model, ME, RMSE, MAE, MPE, MAPE, MASE, RMSSE, ACF1) %>%
