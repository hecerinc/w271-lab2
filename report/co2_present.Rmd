...
```{r Compare against the realised values QUAD, echo=FALSE}
...
```{r Compare against the realised values ARIMA}
# 1. Forecast using the model until Dec 2022 (from 1997 = 25 years)
...
For the NSA data, we fit two ARMIA models, ARIMA(1,0,1)(1,1,0)[52] based on the best (lowest) AIC (AIC = 3524) and ARIMA(1,0,2)(1,1,0)[52] based on the best (lowest) BIC (BIC = 3423) from all permutations of ARIMA models with up to 10 autoregressive processes, 2 unit roots differences, 10 moving average processes, and the equivalent in seasonality components. We used similar techniques to fit the SA data, and found that the same ARIMA model, ARIMA(0,1,2)(1,0,0)[52], had the lowest AIC (AIC = 1841) and BIC (BIC = 1870) for the SA data (model shown below).
...
bind_cols(data.frame(
	model=c('ARIMA(1,0,1)(1,1,0)[52] (NSA data)', 'ARIMA(0,1,2)(1,0,0)[52] (SA data)', 'ARIMA(1,0,1)(1,1,0)[52] (NSA data)', 'ARIMA(0,1,2)(1,0,0)[52] (SA data)'),
...
bind_rows(nsa.accuracy.forecast, sa.accuracy.forecast) %>% 
  ...
  knitr::kable(caption = "Present ARIMA Model Forecasting Accuracy 2020-2022")
```
