...
p1 <- ggplot() +
  ...
  geom_hline(yintercept = 420, color = "steelblue") +
	...
	theme(axis.text.x=element_text(angle=90, vjust=0.5)) +
	labs(title = "Figure 5", 
	  subtitle="ARIMA model forecast, quadratic model forecast, and realised values 1997-2022", x=NULL, y="CO2 ppm") +
	scale_color_discrete(name = "Series")
p1
```
...
rbind(arima_accuracy, quad_accuracy) %>% 
  ...
  knitr::kable(caption = "Table 1: 1997 Model Forecasting Accuracy 1997-2022")
```
...
For the NSA data, we fit two ARMIA models, ARIMA(1,0,4)(1,1,0)[52] based on the best (lowest) AIC (AIC = 3377) and ARIMA(1,0,2)(1,1,0)[52] based on the best (lowest) BIC (BIC = 3423) from all permutations of ARIMA models with up to 10 autoregressive processes, 2 unit roots differences, 10 moving average processes, and the equivalent in seasonality components. We used similar techniques to fit the SA data, and found that the same ARIMA model, ARIMA(0,1,2)(1,0,0)[52], had the lowest AIC (AIC = 1841) and BIC (BIC = 1870) for the SA data (model shown below).

```{r ARIMA models SA data}
...
bind_rows(
  ...
  knitr::kable(caption = "Table 2: Evaluating ARIMA model accuracy for present data (Training Data)")
```

Based on the in-sample accuracy comparisons, for non seasonally adjusted data, we chose the ARIMA(1,0,4)(1,1,0) model because it had slightly better (lower) root mean squared error and ACF1 compared to the other ARIMA model (ARIMA(1,0,2)(1,1,0)). For the seasonally adjusted data, we only had one ARIMA model, ARIMA(0,1,2)(1,0,0), and use that model for out-sample evaluation.

To evaluate our chosen ARIMA models further, we used each model (for NSA and SA sata respectively) to forecast 2 years in the future (2020-2022) and compared the accuracy of each model against our test data sets (both for NSA and SA data). These results are described below in Table 3.

```{r Out of sample NSA and SA accuracy}
#NSA
# Using the ARIMA(1,0,4)(1,1,0) (chosen by in-sample accuracy data)
...
#SA
# Using the ARIMA(1,0,2)(1,1,0) (only model for SA data)
...
bind_cols(data.frame(
	model=c('ARIMA(1,0,4)(1,1,0)[52] (NSA data)', 'ARIMA(0,1,2)(1,0,0)[52] (SA data)')),
bind_rows(
	nsa.accuracy.forecast, sa.accuracy.forecast
)) %>%
  select(model, ME, RMSE, MAE, MPE, MAPE, ACF1) %>%
  knitr::kable(caption = "Table 3: Present ARIMA Model Forecasting Accuracy 2020-2022")
```

Based on Table 3, our ARIMA model for non-seasonally adjusted data outperforms our ARIMA model for seasonally adjusted data in all metrics. 
...
sa.poly.accuracy <- sa.poly.fc %>% fabletools::accuracy(select(test.df, week, season_adjust)) 
```

```{r Test accuracy table SA poly model, echo=FALSE}
bind_cols(data.frame(
  model=c('TSLM(season_adjust ~ trend() + I(trend()^2))', 'ARIMA(0,1,2)(1,0,0)[52] (SA data)')),
  bind_rows(
    sa.poly.accuracy, sa.accuracy.forecast)) %>%
  select(model, ME, RMSE, MAE, MPE, MAPE, ACF1) %>%
  knitr::kable(caption = "Table 4: Present SA Models Forecasting Accuracy 2020-2022")
```
...
nsa.fc <- fabletools::forecast(autofit.arima.bic, h="103 years")
...
mutate(values.2122,
	   ...
	   ci.95.lower = X95.$lower,
	   ci.95.upper = X95.$upper
) %>% select(-X80., -X95.) %>% select(week, .mean, starts_with('ci')) #%>% knitr::kable()
```
