...
authors: 'Carolyn Dunlap, Ayda Nayeb Nazar, Qian Qiao, Hector Rincon'
geometry: margin=1in
output:
...
p3_acf <- ts_co2 %>%
  ...
  labs(title = "Figure 1C", 
    subtitle=TeX(r'(ACF of Monthly Mean $CO_2$)'))

p4_pacf <- ts_co2 %>%
  PACF(value) %>%
  autoplot() +
  labs(title = "Figure 1D", 
    subtitle=TeX(r'(PACF of Monthly Mean $CO_2$)'))

p2_histogram <- ts_co2 %>%
  ggplot(aes(x = value)) +
  geom_histogram() +
  labs(title = "Figure 1B", 
    subtitle=TeX(r'(Histogram of Monthly Mean $CO_2$)'),
        x = TeX(r'($CO_2$ parts per million)'))
#seasonal graph
seasonal_graph <- ts_co2 %>%
  gg_season(value) +
  labs(title = "Figure 2", 
    subtitle = TeX(r'(Monthly Mean $CO_2$ per Year Seasonal Plot)'),
    x = 'Month',
    y = TeX(r'($CO_2$ parts per million)')
  )
...
```
Based on the time series of CO2 levels, we observe that there is both a clear upward trend in overall CO2 levels over time as well as potentially some cyclical seasonal patterns (Fig 1a). Both the seasonal patterns and increasing trend in CO2 ppm is readily apparent in the seasonal plot (Fig 2), where each year there is a peak in CO2 levels around May and a trough of CO2 levels around September-October. There is a strong autocorrelation that has very little decay in the data, and partial autocorrelation is almost 1 and then drops sharply (Fig 1c and d). Altogether, these data indicate that there is a non-stationary linear trend as well as potentially an autoregressive and seasonal component to the data. This is further confirmed in Figure 3, where we decomposed the observed CO2 time series into a trend, seasonal component, and what the leftover time series looks like when we remove those aspects. The trend and seasonal components highlighted in Figure 3 confirms our previous observations and the remaining aspects of the time series seen in the bottom-most plot, does indeed resemble a white-noise process, suggesting that both the trend and seasonal components together approximate the data fairly well.

```{r decomposition plot, echo = FALSE}
plot(decompose(co2))
